...
class VizdoomScenarioEnv(VizdoomEnv):
    ...
    def __init__(
        self, scenario_file, frame_skip=1, client_args="", host = False,num_players = 1, port = 5060, test=False
    ):
        super(VizdoomScenarioEnv, self).__init__(
           os.path.join(scenarios_path, scenario_file), frame_skip, client_args, host,num_players,port , test
        )
