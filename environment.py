...
def create_env(env_name=config.game_name+config.env_type, clip_rewards=True,multi_conf="",is_host =False,testing=False,port=5060):

    env = gym.make(env_name,frame_skip=config.frame_skip,client_args=multi_conf,host=is_host,num_players=config.num_players,test=testing,port=port)
